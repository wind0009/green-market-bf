...
const AdminDashboard: React.FC<{ user: User }> = ({ user }) => {
    ...
    const toggleVendorStatus = async (targetUser: User) => {
        const newStatus = targetUser.isVendor ? false : true;
        await userService.updateUser(targetUser.id, { isVendor: newStatus });
        setUsers(prev => prev.map(u => u.id === targetUser.id ? { ...u, isVendor: newStatus } : u));
    };
